...
def test_sub():
    ...
    with pytest.raises(NotImplementedError):
        sub("b", "B", "abcb", ignore_case=True, __ast_fallback="normal")
...
def test_paste():
    ...
    assert_equal(paste(None, 1, 2, sep="."), NA)
    assert_iterable_equal(
...
def test_strtoi():
    ...
    assert_equal(strtoi("10"), 10)
    assert_iterable_equal(strtoi(["1", "2"]), [1, 2])
    assert_iterable_equal(strtoi(["a", "2"]), [NA, 2])
...
def test_nchar():
    assert_equal(nchar(pa.scalar("abc")), 3)
    ...
    assert_equal(nchar(None, keep_na=True), NA)
    assert_equal(nchar(None, keep_na=False), 2)
    assert_iterable_equal(nchar(pa.array(["abc"])), [3])
    assert_iterable_equal(nchar(np.array(["abc"])), [3])
