...
def test_rep_errors(caplog):
    with caplog.at_level("WARNING"):
        rep(1, length=[1, 2], __ast_fallback="normal")
        assert (
    ...
    with pytest.raises(ValueError):
        rep([1, 2, 3], times=[1, 2], __ast_fallback="normal")

    with pytest.raises(ValueError):
        rep([1, 2, 3], times=[1, 2, 3], each=2, __ast_fallback="normal")
...
def test_order():
    ...
    assert_iterable_equal(order([3, 2, NA, 1]), [3, 1, 0, 2])
    assert_iterable_equal(
        order([3, 2, NA, 1], na_last=False),
        [2, 3, 1, 0],
    )
    assert_iterable_equal(
        order([3, 2, NA, 1], na_last=False, decreasing=True),
        [2, 0, 1, 3],
    )
...
def test_sort():
    ...
    assert_iterable_equal(sort([3, 2, NA, 1]), [1, 2, 3, NA])
    assert_iterable_equal(
        sort([3, 2, NA, 1], na_last=False),
        [NA, 1, 2, 3],
    )
    assert_iterable_equal(
        sort([3, 2, NA, 1], na_last=False, decreasing=True),
        [NA, 3, 2, 1],
    )
...
def test_seq_len():
    ...
    assert_iterable_equal(seq_len([pa.scalar(3)]), [1, 2, 3])
